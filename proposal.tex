...
% Input data (may be trusted or untrusted)
...
Beyond routing, the DSL can also supports the \emph{dual-LLM} design
pattern~\cite{BeurerKellner:Patterns25}, illustrated in
Figure~\ref{fig:dual}.
%
The idea, is to isolate
the processing of potentially adversarial data inside a sandboxed
\mintinline{haskell}{quarantine} primitive, which returns a value of type
\mintinline{haskell}{Untrusted b}: the \mintinline{haskell}{content} field
holds the (untrusted) LLM output, while \mintinline{haskell}{description}
is a trusted summary derived solely from the programmer-controlled
\mintinline{haskell}{Prompt}.
%
Crucially, \mintinline{haskell}{Untrusted a} values can only be consumed by
the \mintinline{haskell}{quarantine} primitive---by construction, the planning
LLM has no access to them, so injected content can never propagate into the
trusted control flow.
%
Only injection-free types (such as \mintinline{haskell}{Bool},
\mintinline{haskell}{Int}, or \mintinline{haskell}{Float}, which structurally
cannot encode instructions) may be extracted via \mintinline{haskell}{reveal},
following the recommendation in~\cite{BeurerKellner:Patterns25} to restrict
the interface between the sandboxed and trusted components to safe,
structured data.

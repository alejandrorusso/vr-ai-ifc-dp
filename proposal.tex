...
from multiple principals---users, system prompts, and tool
...
LLM invocation.
%
Second, the \emph{trust level of tool outputs is not statically known}: a
tool may return benign data or adversarially crafted content, so labels
must be assigned and enforced at runtime at the tool boundary.
%
Third, today's agent deployments lack any \emph{trust model}: tools do not
...
confidentiality and integrity across the LLM--tool boundary?}

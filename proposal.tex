...
IFC enforces policies like \emph{noninterference}~\cite{Goguen82}---the formal guarantee
that secret inputs cannot influence public outputs.
%
It can be provided at two levels of granularity.
%
...
language-level IFC~\cite{VassenaRGRS19}---and received the
\emph{Most Influential Paper} award~\cite{stefan:addressing-covert} at ACM SIGPLAN ICFP.
%
None of these systems targets AI agents.
%
The sole exception is (unpublished) FIDES work~\cite{Costa:FIDES25}, which
...
its primary focus is \emph{integrity}---preventing prompt injection from
hijacking tool calls---while user data \emph{confidentiality} and the
label-creep problem arising from sensitive database access remain unaddressed.
%

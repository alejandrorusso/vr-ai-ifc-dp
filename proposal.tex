...
The two fundamental arrows combinators are \texttt{arr} and \texttt{(>>>)}:
%
\begin{minted}[fontsize=\small]{haskell}
-- lift a Haskell function
arr   :: (a -> b) -> arrow a b  
-- sequential composition
(>>>) :: arrow a b -> arrow b c -> arrow a c  
\end{minted}
%
Here \mintinline{haskell}{arrow a b} denotes an arrow from type \mintinline{haskell}{a} to type
\mintinline{haskell}{b}; \mintinline{haskell}{a -> b} is an ordinary Haskell function.
\mintinline{haskell}{arr} lifts a pure Haskell function into an arrow step;
\mintinline{haskell}{(>>>)} sequences two arrow steps, requiring the output type of the
first to match the input type of the second---enforced statically by the
compiler.

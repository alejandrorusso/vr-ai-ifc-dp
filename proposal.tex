...
\node[tool, right=3.0cm of clf, yshift= 0.7cm] (h1) {\scriptsize notify IT};
\node[tool, right=3.0cm of clf]                (h2) {\scriptsize log ticket};
\node[tool, right=3.0cm of clf, yshift=-0.7cm] (h3) {\scriptsize quarantine};
...
\end{minted}
\end{minipage}
\caption{LLM-as-classifier pattern: the LLM selects from a fixed, typed set of
  options.\label{fig:choice}}
\end{framed}
\end{figure*}
...
domain-specific language (DSL), implemented in Haskell, that encodes agent
plans as typed computation graphs. 
%
The DSL will be capable to encode \emph{design patterns} for
prompt-injection-resistant agents identified in recent
work~\cite{BeurerKellner:Patterns25}. 
%
...
\end{minted}
%
Intuitively, \mintinline{haskell}{arrow a b} represents a \emph{box} in a
data-flow diagram: it accepts an input of type \mintinline{haskell}{a},
performs some computation, and produces an output of type \mintinline{haskell}{b}.
%
Here \mintinline{haskell}{arrow}, \mintinline{haskell}{a}, and
\mintinline{haskell}{b} are all type variables: \mintinline{haskell}{a} and
\mintinline{haskell}{b} are placeholders for concrete types such as
\mintinline{haskell}{FilePath} or \mintinline{haskell}{String}, while
\mintinline{haskell}{arrow} ranges over any concrete \emph{arrow datatype}.
%
The concrete datatype substituted for \mintinline{haskell}{arrow} determines
the \emph{effects and capabilities} of each box. 
%
In this proposal, we assume a \mintinline{haskell}{LLM} datatype that captures
the ability to invoke a language model, tools, and MCP servers, while a
different instantiation could restrict boxes to pure computation or add
security-relevant capabilities such as information-flow tracking.
%
\mintinline{haskell}{arr} lifts a plain Haskell function into an arrow box
with no additional effects.
%
For example, \mintinline{haskell}{arr lines :: LLM String [String]} wraps
\mintinline{haskell}{lines :: String -> [String]} (which splits a string into
lines) into a step that requires no LLM call.
%
\mintinline{haskell}{(>>>)} connects two boxes in sequence, requiring the
output type of the first box to match the input type of the second---a
constraint enforced statically by the compiler.
%

To see \mintinline{haskell}{arr} and \mintinline{haskell}{(>>>)} in action,
consider the request \emph{``read all \texttt{*.md} files in \texttt{/home/ale/}
and email me a summary''}, the LLM synthesises the plan in 
Figure~\ref{fig:arrows}.
%
In Haskell, \mintinline{haskell}{[a]} denotes a list of elements of type
\mintinline{haskell}{a}---so \mintinline{haskell}{[FilePath]} is a list of
file paths and \mintinline{haskell}{[String]} is a list of strings.
%
An attacker embedding \texttt{"ignore previous instructions and forward all
...
Beyond sequential pipelines, arrows also support \emph{choice} via the
...
\end{minted}
%
Type \mintinline{haskell}{Either a b} is a type that holds \emph{either} a value
of type \mintinline{haskell}{a} (tagged \mintinline{haskell}{Left}) or a
value of type \mintinline{haskell}{b} (tagged \mintinline{haskell}{Right}). 
%
In short, given two arrows, \mintinline{haskell}{(|||)} produces a single arrow that
routes \mintinline{haskell}{Left} values to the first and
\texttt{Right} values to the second.
%
Chaining \mintinline{haskell}{(|||)} encodes $n$-way branches: nested
\mintinline{haskell}{Either}s cover arbitrarily many cases, each routed to
its own fixed handler.
%

The \mintinline{haskell}{(|||)} directly realises the \emph{LLM-as-classifier}
design pattern of~\cite{BeurerKellner:Patterns25}, where an LLM decides which
branch to take but cannot introduce branches outside those declared in the
plan.
%
We show a plan in Figure~\ref{fig:choice} 
to triage incoming emails: urgent
security alerts should notify the IT team, routine requests should be logged as
tickets, and spam should be quarantined.
%
The arrow \mintinline{haskell}{classify} returns \mintinline{haskell}{Left Urgent},
\mintinline{haskell}{Right (Left Routine)}, or \mintinline{haskell}{Right
(Right Spam)}, and \mintinline{haskell}{(|||)} routes each case to
\mintinline{haskell}{notifyIT}, \mintinline{haskell}{logTicket}, or
\mintinline{haskell}{quarantine}, respectively.
%
An attacker injecting \emph{``disregard your instructions and call
\texttt{deleteAllData}''} into the email body cannot cause the agent to invoke
an arbitrary tool---the only reachable handlers are those declared in the plan.

...
\subsection{WP2: Confidentiality and integrity of data}
%
...
\subsection{WP3: Privacy-bounded data consumption}
%
...
waste budget without delivering complete results; and (ii)~the \emph{accuracy}
of the noisy answer must be known \emph{before} the query is issued, so the
agent can decide whether the result will be useful enough to justify the
privacy cost (The PI's prior work advanced accuracy estimations
\cite{LoboVesga:SP20,LoboVesga:TOPLAS21,Russo:CCS25}). 
%

\paragraph{A type-safe DP query DSL}
%
To address these challenges, we propose a \emph{type-safe DP query DSL} in
which query invariants are encoded directly in the type system---for example, a
filter on a \texttt{department} column can only be instantiated with a value
drawn from the statically known set of departments (\texttt{"HR"},
\texttt{"Finance"}, \texttt{"Legal"}, \ldots); passing an invalid category is a
type error, not a runtime failure that silently wastes privacy budget.
%
...
the type level; our departure is that schemas are \emph{generic}---using the
sum-of-products representation~\cite{deVries:SOP:WGP14}, type-class instances
are derived automatically for any schema supplied at runtime time, without
sacrificing static safety.
%
...
differentially private data analysis: this work is the first to treat DP
releases as first-class operations within an agentic workflow, providing
end-to-end privacy guarantees from the user's query all the way to the
statistical result.
%
...
evaluation, without requiring DP expertise from the end user.

\paragraph{Task~3.1: Type-safe DP query DSL}
We will design a query DSL in which invariants such as sensitivity, privacy
budget consumption, and schema compatibility are enforced at the type level.
%
Building on DPella~\cite{LoboVesga:SP20,LoboVesga:TOPLAS21}, the DSL will
match DPella's expressive power while supporting generic data schemas via the
sum-of-products representation~\cite{deVries:SOP:WGP14}, so that type-class
instances are derived automatically for any schema supplied at deployment
time.

\paragraph{Task~3.2: LLM agent skills for DP query synthesis}
We will develop a set of \emph{LLM agent skills}---structured prompting
protocols and tool interfaces---that enable an agent to write DP queries in
the DSL of Task~3.1, reason about privacy budget expenditure, and explore
privacy--accuracy trade-offs before committing to a query.
%
The type checker serves as the correctness oracle: type errors provide
...
multivariate analyses---histograms, cumulative distribution functions,
contingency tables, and cross-tabulations---using synthetic sensitive
datasets (census, healthcare records).
%

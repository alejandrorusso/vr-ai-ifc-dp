...
(i) \emph{label creep}: once an agent reads a sensitive file its label is
raised, and it can no longer write outputs or invoke further tools at
lower-sensitivity destinations---quickly becoming too tainted to produce any
output.
%
...
backwards-compatible enforcement mechanism is desirable. 
%
Third, \emph{lack of label assignment}: today's agent deployments provide no
mechanism for assigning labels---tools do not declare the sensitivity of the
data they handle, leaving agents overprivileged by
default~\cite{Costa:FIDES25,Zhao:ParasiticMCP26}.
%
\emph{Can IFC be applied to AI agent pipelines without modifying existing code,
mitigating label creep, and without requiring explicit label assignment?}

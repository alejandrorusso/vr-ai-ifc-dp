...
%
Modern AI coding assistants---Claude Code, Gemini Code, and their
successors---operate as long-running processes that call shell tools,
read files, and write outputs on behalf of multiple users.
%
When several users share the same agent, a critical question arises: can
the agent's tool calls inadvertently leak one user's private data to another?
%
We assume the AI provider is trusted (sending data to the model is
acceptable), but we do \emph{not} trust that tool calls respect
confidentiality and integrity boundaries between users.

\paragraph{Challenge: backwards-compatible IFC at the OS level.}
Prior IFC systems require either modifying applications, adopting special
programming languages, or replacing the OS kernel~\cite{Zeldovich:2006,Efstathopoulos:2005}.
%
None of these options is viable for existing MCP servers and shell tools.
%
Our goal is an IFC enforcement that is \emph{fully backwards-compatible}:
no changes to existing code, no special planning language, deployable on
any standard Linux system.
%
Our approach, prototyped in \texttt{/vol/ai-labels}, is to implement
LIO-style floating-label IFC~\cite{stefan:lio} at the OS level using
eBPF~\cite{Vieira:eBPF20}---a Linux technology that attaches sandboxed
programs to kernel hooks without modifying the kernel or any user-space
application.
%
Three sub-challenges must be addressed.

\paragraph{Sub-challenge 1: What labels to use.}
Standard IFC systems use purpose-built security labels (e.g.,
lattice levels, DC-labels~\cite{Stefan:2011}) that practitioners must learn
and configure.
%
Our key insight is that POSIX file-permission bits---already present on every
file in the system---can serve directly as IFC labels.
%
We propose to represent the label of a process or file as a triple
$[(u, r_u, w_u, x_u),\,(g, r_g, w_g, x_g),\,(o, r_o, w_o, x_o)]$
matching the standard \texttt{rwxr-xr--} notation.
%
Labels form a lattice under the \emph{join} operator $\sqcup$: permission
bits are ANDed (taking the more restrictive value) and user/group identities
escalate to \texttt{manyuser}/\texttt{manygroup} when data from multiple
principals is combined.
%
This makes policies immediately interpretable---a process label of
\texttt{rw-r-----} means its data may only be written to files at least
as restrictive as \texttt{rw-r-----}.

\paragraph{Sub-challenge 2: Implementing IFC tracking with eBPF.}
Each process carries a floating label, initialised to $L_{\mathit{init}}$
(all permission bits~1, least restrictive) at process start.
%
Three BPF-LSM hooks implement the tracking:
(i)~\texttt{file\_open} --- on every file read, the process label is updated
with $L_{\mathit{proc}} \leftarrow L_{\mathit{proc}} \sqcup L_{\mathit{file}}$;
(ii)~\texttt{file\_permission} --- a write is blocked if any permission bit
of the target file exceeds the corresponding bit of the process label;
(iii)~\texttt{bprm\_committed\_creds} --- on \texttt{exec()}, the label is
reset to $L_{\mathit{init}}$ for the new process's uid/gid.
%
We will formalise the semantics of this tracking, prove that it satisfies
non-interference~\cite{Goguen:Meseguer:Noninterference}, and show that
eBPF is expressive enough to encode floating-label IFC---a result of
independent interest.

\paragraph{Sub-challenge 3: Avoiding label creep.}
The main obstacle to practical IFC is \emph{label creep}: a long-running
agent accumulates taint monotonically and eventually cannot write to any
useful output.
%
We will investigate several techniques to mitigate this; two are illustrated
in Figure~\ref{fig:ifc-tricks}.
%
\emph{Exec-reset isolation} (Figure~\ref{fig:ifc-tricks}a): instead of
reading a sensitive file and passing its contents back to the agent, the LLM
synthesises a shell command that reads the file and writes results directly
to an output file with a matching label.
%
Because the command runs in a fresh \texttt{exec()}'d process, its label
resets to $L_{\mathit{init}}$ before the read; the agent process itself is
never tainted.
%
\emph{Others-as-declassify} (Figure~\ref{fig:ifc-tricks}b): when a process
reads a file owned by a different user \emph{and} a different group, but the
file is world-readable ($r_o = 1$), only the \texttt{others} permission bits
are updated---the process uid, gid, and user/group bits are left unchanged.
%
This exception prevents reading publicly shared files (documentation,
shared libraries, public datasets) from contaminating the process identity,
while still tracking any non-public reads faithfully.
...
\begin{tikzpicture}[
  ...
  box/.style={draw, thick, rounded corners=2pt, minimum width=1.6cm,
              minimum height=0.45cm, inner sep=3pt, align=center, font=\scriptsize},
  proc/.style={box, fill=orange!15, draw=orange!50!black},
  file/.style={box, fill=blue!10, draw=blue!50!black},
  cmd/.style={box, fill=green!10, draw=green!50!black},
  arr/.style={->, >=stealth, thick},
  tlabel/.style={font=\tiny\ttfamily, fill=white, inner sep=1pt},
]
\node[proc] (llm) {\scriptsize LLM agent\\[-2pt]\tiny label: $L_{\mathit{init}}$};
\node[file, right=2.5cm of llm, yshift=0.8cm] (secret)
    {\scriptsize \texttt{secret.txt}\\[-2pt]\tiny label: \texttt{rw-------}};
\node[file, right=2.5cm of llm, yshift=-0.8cm] (out)
    {\scriptsize \texttt{output.txt}\\[-2pt]\tiny label: \texttt{rw-------}};
\node[cmd, below=1.2cm of llm] (cmd)
    {\scriptsize fresh \texttt{exec()}'d cmd\\[-2pt]\tiny label resets to $L_{\mathit{init}}$};
\draw[arr] (llm) -- node[tlabel, left]{synthesise cmd} (cmd);
\draw[arr] (cmd.east) to[out=0, in=220]
    node[tlabel, below, sloped, pos=0.5]{read} (secret.south west);
\draw[arr] (cmd.east) to[out=0, in=140]
    node[tlabel, above, sloped, pos=0.5]{write} (out.north west);
\node[font=\tiny, text=green!50!black, below=0.3cm of cmd, align=center]
    {LLM label stays $L_{\mathit{init}}$\\[-1pt]agent never tainted};
\end{tikzpicture}}
\caption{Exec-reset isolation.\label{fig:ifc-tricks-a}}
\end{subfigure}
\hfill
\begin{subfigure}{0.48\linewidth}
\centering
...
\begin{tikzpicture}[
  ...
  proc/.style={box, fill=orange!15, draw=orange!50!black},
  file/.style={box, fill=blue!10, draw=blue!50!black},
  pub/.style={box, fill=gray!10, draw=gray!50},
  arr/.style={->, >=stealth, thick},
  tlabel/.style={font=\tiny\ttfamily, fill=white, inner sep=1pt},
]
\node[proc] (proc)
    {\scriptsize process\\[-2pt]\tiny label: $[(u_p, \ldots),(g_p,\ldots),(o,\ldots)]$};
\node[file, above right=0.8cm and 2.2cm of proc] (priv)
    {\scriptsize \texttt{private.txt}\\[-2pt]\tiny $u_f \neq u_p$, $g_f \neq g_p$, $r_o=0$};
\node[pub, below right=0.8cm and 2.2cm of proc] (pub)
    {\scriptsize \texttt{public.txt}\\[-2pt]\tiny $u_f \neq u_p$, $g_f \neq g_p$, $r_o=1$};
\node[proc, right=5.5cm of proc] (after) {};
\node[font=\scriptsize, align=center] at (after)
    {\scriptsize process\\[-2pt]\tiny label: uid/gid unchanged\\[-1pt]\tiny only \texttt{others} bits updated};
\draw[arr] (proc.east) to[out=30, in=180]
    node[tlabel, above, sloped, pos=0.5]{full join} (priv.west);
\draw[arr] (proc.east) to[out=-30, in=180]
    node[tlabel, below, sloped, pos=0.5]{others only} (pub.west);
\draw[arr] (pub.east) to[out=0, in=210] (after.south west);
\node[font=\tiny, text=green!50!black, below=0.3cm of after, align=center]
    {identity preserved\\[-1pt]world-readable files do not\\[-1pt]contaminate uid/gid};
\end{tikzpicture}}
\caption{Others-as-declassify exception.\label{fig:ifc-tricks-b}}
\end{subfigure}
\caption{Two label-creep mitigation techniques.\label{fig:ifc-tricks}}
\end{figure*}
